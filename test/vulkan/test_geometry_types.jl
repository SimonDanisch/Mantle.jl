...
@testset "pack_geometry! :: TrianglesGeometry byte layout" begin
    ...
    Mantle.pack_geometry!(buf, 0, geom; geo_flags=UInt32(1))   # OPAQUE_BIT

    # Outer VkAccelerationStructureGeometryKHR header
    @test reinterpret(Int32,  buf[1:4])[1]   == Mantle.VK_STYPE_GEO     # sType @ 0
    ...
    @test reinterpret(Int32,  buf[25:28])[1] == Mantle.VK_STYPE_TRI     # sType @ q+0
...
@testset "pack_geometry! :: AABBsGeometry byte layout" begin
    ...
    Mantle.pack_geometry!(buf, 0, geom; geo_flags=UInt32(1))

    @test reinterpret(Int32,  buf[1:4])[1]   == Mantle.VK_STYPE_GEO    # sType @ 0
    @test reinterpret(UInt32, buf[17:20])[1] == UInt32(1)             # geometryType = AABBS @ 16
    @test reinterpret(Int32,  buf[25:28])[1] == Mantle.VK_STYPE_AABB   # sType @ q+0
...
@testset "AABB BLAS - query_as_build_sizes returns nonzero" begin
    dev = Mantle.vk_context().device
    geom = AABBsGeometry(; aabb_addr=UInt64(0), aabb_stride=UInt64(24))
    sizes = Mantle.query_as_build_sizes(dev, geom;
        as_type = UInt32(1),
...
@testset "AABB BLAS - build_blas_aabb smoke" begin
    ...
    blas = Mantle.build_accel!(Mantle.vk_context().default_bq) do ctx
        Mantle.build_blas_aabb(ctx, aabbs)
    end
    @test blas isa Mantle.LavaBLAS
...
@testset "AABB BLAS - build_blas_aabb errors loudly without ray_query" begin
    ...
    try
        @test_throws ErrorException Mantle.build_accel!(Mantle.vk_context().default_bq) do bc
            Mantle.build_blas_aabb(bc, [AABB(Point3f(0, 0, 0), Point3f(1, 1, 1))])
        end
    finally
