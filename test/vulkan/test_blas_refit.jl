...
@testset "BLAS refit moves the acceleration structure" begin
    ...
    if ctx.rt_pipeline_properties === nothing
        ...
        blas, tlas = Mantle.as_build() do c
        ...
        pipeline = Mantle.create_rt_pipeline(ctx, build_raygen_shader(),
        ...
        function trace_center()
            Mantle.rt_dispatch!(ctx.default_bq, pipeline, tlas, output_buf.address, W, H)
            bytes = Vector{UInt8}(undef, W * H * sizeof(Float32))
            Mantle.download!(bytes, output_buf)
