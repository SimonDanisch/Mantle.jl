...
@testset "recorded dispatches synchronize mapped inputs" begin
    ...
    Mantle.compute!(g, "mapped input") do p
        Mantle.dispatch!(p, recorded_read_scalar!,
                         (Mantle.use(p, dst; write = true),
                          Mantle.use(p, src; read = true)), length(dst); group = 64)
    end
    ...
    try
        ...
        Mantle.compute!(writergraph, "other queue writes input") do p
            Mantle.dispatch!(p, recorded_read_scalar!,
                             (Mantle.use(p, src; write = true),
                              Mantle.use(p, dst; read = true)), 1; group = 1)
        end
...
@testset "partitioned recordings preserve dependencies and replay" begin
    ...
    for k in 1:10
        Mantle.compute!(g, "double$k") do p
            Mantle.dispatch!(p, partition_double!,
                             (Mantle.use(p, a; read = true, write = true),), length(a))
        end
    end
    pl = Mantle.Plan(g)
...
@testset "partitioned recording follows moved inputs" begin
    ...
    g = Mantle.Graph(dev)
    for i in 1:3
        Mantle.compute!(g, "copy$i") do p
            s = Mantle.use(p, src; read=true)
            d = Mantle.use(p, out; write=true)
            Mantle.dispatch!(p, partition_copy!, (d, s), 128)
        end
    end
