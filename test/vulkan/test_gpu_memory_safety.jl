...
@testset "GPU Memory Safety" begin
    ...
    @testset "dispatch + free + flush cycles" begin
        ...
        @test Mantle.gpu_live_bytes() <= baseline_bytes + 16 * 1024^2   # 16 MiB ceiling
    end
    ...
    @testset "vk_reduce_sum scratch is cached" begin
        drain!()
        baseline = Mantle.gpu_live_bytes()
        a = Mantle.LavaArray(rand(Float32, 100_000))
        for _ in 1:1000
            Mantle.vk_reduce_sum(a)
        end
        drain!()
        # One call may allocate a 4-byte scratch the first time. After that it
        # must reuse the same LavaArray forever.
        @test Mantle.gpu_live_bytes() <= baseline + 1 << 20  # generous 1 MiB ceiling
        Mantle.unsafe_free!(a)
    end
    ...
    @testset "pool blocks bounded under bursty alloc" begin
        ...
        for _ in 1:50
            a = Mantle.LavaArray{Float32}(undef, 2_000_000)
            Mantle.unsafe_free!(a)
        end
        drain!()
    ...
    @testset "the retired list drains" begin
        drain!()
        for _ in 1:10
            Mantle.LavaArray(Float32[1, 2, 3, 4])  # orphaned, will be GC'd + deferred
        end
        GC.gc(true)
        drain!()
    ...
    @testset "derived arrays keep parent alive" begin
        @testset "GPUArrays.derive shares the DataRef" begin
            parent = Mantle.LavaArray(Float32[10, 20, 30, 40, 50])
            ...
            @test child.buf[].address != Mantle.BDA_POISON
        ...
        @testset "reshape shares the DataRef" begin
            a = Mantle.LavaArray(Float32[1, 2, 3, 4, 5, 6])
            b = reshape(a, 2, 3)
            @test a.buf[] === b.buf[]
            Mantle.unsafe_free!(a)
    ...
    @testset "launch arg validation toggle" begin
        ...
        Mantle.vk_context().diag.launch_arg_validation = false
        try
            @test Mantle.vk_context().diag.launch_arg_validation == false
        finally
            Mantle.vk_context().diag.launch_arg_validation = true
        end
        @test Mantle.vk_context().diag.launch_arg_validation == true
    end
    ...
    @testset "VulkanBatchQueue state bounded" begin
        ...
        before = sum(b -> length(b.live), Mantle.unifiedblocks(Mantle.ctxof(BQ)); init = 0)
        ...
        @test length(Mantle.unifiedblocks(Mantle.ctxof(BQ))) <= 4
        @test sum(b -> length(b.live), Mantle.unifiedblocks(Mantle.ctxof(BQ)); init = 0) == before
