...
@testset "batched coopmat GEMM" begin
    if !Mantle.coopmat_gemm_available()
        @info "skipping: the coopmat GEMM needs a $(Mantle.GEMM_SUBGROUP)-lane subgroup and needs the device to report a $(Mantle.GEMM_TILE)^3 Float16 shape; this device has subgroup=$(Mantle.device_subgroup_size()), shape=$(Mantle.coopmat_shape(Mantle.vk_context(), Float16, Mantle.GEMM_TILE, Mantle.GEMM_TILE, Mantle.GEMM_TILE))"
    else
