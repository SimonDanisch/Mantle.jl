...
@testset "Atomic GPU Execution" begin
    ...
    @testset "atomic counter (Int32)" begin
        ...
        atomic_counter_i32!(Mantle.defaultbackend())(counter; ndrange=N)
    ...
    @testset "atomic counter (UInt32)" begin
        ...
        atomic_counter_u32!(Mantle.defaultbackend())(counter; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "atomic counter (Float32 CAS)" begin
        ...
        atomic_counter_f32!(Mantle.defaultbackend())(counter; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "atomic returns unique values" begin
        ...
        atomic_unique!(Mantle.defaultbackend())(counter, results; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "cross-workgroup atomic visibility (BVH refit pattern)" begin
        ...
        refit_pattern!(Mantle.defaultbackend())(data, flags, results, parents_d, siblings_d; ndrange=N)
        Mantle.flush!(Mantle.Device())
...
@testset "Batched Dispatch" begin
    ...
    @testset "barrier between dispatches preserves ordering" begin
        ...
        write_val!(Mantle.defaultbackend())(A, 42.0f0; ndrange=N)
        read_add!(Mantle.defaultbackend())(B, A; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "data kept alive across GC + flush" begin
        ...
        fill_index!(Mantle.defaultbackend())(A; ndrange=1024)
        GC.gc()
        Mantle.flush!(Mantle.Device())
        result = Array(A)
        @test result[1] == 1.0f0
...
@testset "KA @private (Scratchpad)" begin
    ...
    private_accum!(Mantle.defaultbackend(), 64)(a; ndrange=64)
    Mantle.flush!(Mantle.Device())
    result = Array(a)
...
@testset "Int64/UInt64 atomics" begin
    @testset "Int64 atomic add" begin
        ...
        atomic_add_i64!(Mantle.defaultbackend())(c; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "UInt64 atomic add" begin
        ...
        atomic_add_u64!(Mantle.defaultbackend())(c; ndrange=N)
        Mantle.flush!(Mantle.Device())
...
@testset "Float64 atomics" begin
    @testset "Float64 atomic add" begin
        ...
        atomic_add_f64!(Mantle.defaultbackend())(c; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "Float64 atomic subtract" begin
        ...
        atomic_sub_f64!(Mantle.defaultbackend())(c; ndrange=N)
        Mantle.flush!(Mantle.Device())
        @test Array(c)[1] ≈ 0.0
    end
end
...
@testset "Atomix CartesianIndex and Float32 subtract" begin
    ...
    @testset "Float32 atomic subtract" begin
        ...
        atomic_sub_f32!(Mantle.defaultbackend())(counter; ndrange=N)
        Mantle.flush!(Mantle.Device())
    ...
    @testset "Atomix with CartesianIndex on 3D array" begin
        ...
        atomic_add_3d!(Mantle.defaultbackend())(A, idx_array; ndrange=N)
        Mantle.flush!(Mantle.Device())
        result = Array(A)
    ...
    @testset "Atomix subtract with CartesianIndex on 2D array" begin
        ...
        idx_array = Mantle.LavaArray(fill(target, N))
        atomic_sub_2d!(Mantle.defaultbackend())(A, idx_array, 1.0f0; ndrange=N)
        Mantle.flush!(Mantle.Device())
        result = Array(A)
    ...
    @testset "Atomix with N integer indices on 2D array (no CartesianIndex)" begin
        ...
        atomic_add_ij!(Mantle.defaultbackend())(A, is, js; ndrange=N)
        Mantle.flush!(Mantle.Device())
        result = Array(A)
