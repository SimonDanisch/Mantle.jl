...
function chain(dev, n::Integer, links::Integer; alias::Bool = true)
    ...
    for (k, dst) in enumerate(t)
        M.compute!(g, "l$k") do p
            M.dispatch!(p, rocadd!, (M.use(p, dst; write = true),
...
@testset "ROCm: record! writes the plan down and does not run it" begin
    ...
    M.compute!(g, "bump") do p
        M.dispatch!(p, rocadd!, (M.use(p, acc; read = true, write = true),
                                 M.use(p, acc; read = true)), n)
    end
    pl = M.Plan(g)
    M.record!(pl)
    M.waitidle(dev)
...
@testset "ROCm: a move invalidates the recording, and is then refused" begin
    ...
    M.compute!(g2, "one") do p
        M.dispatch!(p, rocadd!, (M.use(p, dst2; write = true),
                                 M.use(p, src; read = true)), n)
    end
...
@testset "ROCm: a compiled dispatch is partitioned the way an eager launch is" begin
    ...
    dst = M.Transient.Buffer(g, Float32, n)
    M.compute!(g, "k") do p
        M.dispatch!(p, roc2d!, (M.use(p, dst; write = true),
                                M.use(p, src; read = true), dims[1]), dims)
    end
    pl = M.Plan(g)
