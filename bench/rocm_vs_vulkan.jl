...
function chainplan(dev, n::Integer, links::Integer; alias::Bool = true, record::Bool = true)
    ...
    for (k, dst) in enumerate(t)
        M.compute!(g, "l$k") do p
            M.dispatch!(p, tinyadd!, (M.use(p, dst; write = true),
                                      M.use(p, prev; read = true)), n)
        end
        prev = dst
    end
    pl = M.Plan(g; alias)
...
function mixed(m1 = 1152, n = 1024, k = 1152, m2 = 1152)
    ...
    M.compute!(g, "gemm1") do p
        M.dispatch!(p, LibOp(gemmop(m1, n, k)),
                    (M.use(p, h; write = true), M.use(p, W1; read = true),
                     M.use(p, X; read = true)), 1)
    end
    M.compute!(g, "epi1") do p
        M.dispatch!(p, biasgelu!, (M.use(p, h; read = true, write = true),
                                   M.use(p, B1; read = true), Int32(m1)), m1 * n)
    end
    M.compute!(g, "gemm2") do p
        M.dispatch!(p, LibOp(gemmop(m2, n, m1)),
                    (M.use(p, o; write = true), M.use(p, W2; read = true),
                     M.use(p, h; read = true)), 1)
    end
    M.compute!(g, "epi2") do p
        M.dispatch!(p, biasgelu!, (M.use(p, o; read = true, write = true),
                                   M.use(p, B2; read = true), Int32(m2)), m2 * n)
    end
    pl = M.Plan(g)
    M.record!(pl)
