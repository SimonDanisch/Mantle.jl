...
# ── What this file is ────────────────────────────────────────────────────────
#
# The scene and the frame: constants, geometry, every compute kernel, every
# shader, every pipeline, and the graph that wires them together. No device, no
# window, no backend — `showcase_plan(dev, screenof)` takes those.
#
# It is split out because the twenty passes below are the demo, and they are the
# same twenty on any backend. `showcase.jl` drives them on Vulkan into a window;
# `showcase_metal.jl` drives them on Metal into an offscreen target. Neither
# copy of the scene could drift from the other, because there is one.
#
# NOTHING here names a backend. `vertex_index()` and `frag_coord_x()` were the
# last two words that did — they were Lava's, and separately Metal's — and they
# are `Mantle`'s now, overridden per backend through GPUCompiler's method table.

import Mantle
using GeometryBasics, LinearAlgebra, KernelAbstractions, Atomix, Random
using ColorTypes: RGBA
using ColorTypes.FixedPointNumbers: N0f8
M = Mantle
# Explicitly, not `using Mantle`: several of these names also exist in
# KernelAbstractions' orbit, and a demo is the wrong place to find that out.
# This list IS the portable shader vocabulary — every word the twenty passes
# below are written in, and not one of them names a backend.
using Mantle: Rasterizer, TriangleList, LineList, Opaque, AlphaBlend,
              NoCull, CullBack, CullFront, DepthOff, DepthLess,
              DrawIndirectCommand,
...
@inline function sunshadow(smap, sunvp::Mat4f, world::Vec3f, n::Vec3f, ndl::Float32,
    ...
    # Through `clip_y`, because this is the viewport transform done by hand and
    # it has to be the one the rasteriser did. The two clip conventions differ
    # in y, every vertex stage's `position` already goes through this, and a
    # reprojection that skips it samples the mirror image of the row it wanted
    # — quietly, because the map is still full of real depths.
    fy = (0.5f0 * clip_y(q[2]) + 0.5f0) * Float32(res)
...
"""
The ambient-occlusion settings, as ONE argument.

Grouped because a backend binds one slot per kernel argument and Metal allows
31 of them: `shade!` reads eleven buffers and wanted twenty-one loose scalars
beside them, which is 32 and one too many — the driver answers "Total number of
indirect argument buffer resources exceeded for buffers (32/31)".

Vulkan pushes its scalars through a push-constant range and never runs out, so
this is not a limit the graph can hide. It is also not a workaround: a uniform
block per subsystem is how a renderer is written anyway, and the same four
values travel together everywhere they are read.
"""
struct AOParams
    w::Int32
    h::Int32
    amb::Float32       # how hard occlusion presses on the ambient
    direct::Float32    # and how much of that reaches the point lights
end
...
const LIGHTHOME = [(a = 2pi * rand(Float32); r = 12f0 + 62f0 * rand(Float32);
                    Vec4f(r * cos(a), 2f0 + 17f0 * rand(Float32)^1.6f0, r * sin(a),
                          24f0 + 22f0 * rand(Float32))) for _ in 1:NLIGHT]

# ── the whole frame, as one function of the device ───────────────────────────

"""
    showcase_plan(dev, screenof) -> NamedTuple

Build every resource, every pass and the compiled plan.

`screenof(graph)` supplies the target the composite pass draws into: a
`M.Surface` over a window on one driver, a `M.Transient.Image` on the other.
...
Returns the plan, the screen, and every `Ref` a frame updates.
"""
function showcase_plan(dev, screenof; profile::Bool = false)
    ...
    lighthome = M.Buffer(dev, LIGHTHOME)
...
"""
function showcase_frame!(s, t, radius, height, freeze)
    ...
    freeze || (s.cullref[] = vp)
    s.invref[] = inv(vp)
    s.eyeref[] = Vec4f(eye[1], eye[2], eye[3], 1f0)
    s.timeref[] = t
    s.aoref[] = AOParams(Int32(AOW), Int32(AOH), s.aoamb[], s.aodirect[])
    M.run!(s.plan)
end
