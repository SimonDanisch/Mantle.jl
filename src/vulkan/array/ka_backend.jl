...
# ── Shared Memory ──
#
# `lava_alloc_shared`, `LavaSharedArray` and the `KA.SharedMemory` override are
# in Lava, at `device/sharedmemory.jl`. They are device code: an LLVM global in
# addrspace(3) and an index into it, with no queue, pool or context anywhere.
# The `CoopMatrix` load/store methods over a shared array are below, with the
# rest of the cooperative-matrix kernels.
# ── Synchronization ──
