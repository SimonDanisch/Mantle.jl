...
#
# Declared, none of that exists. `use(p, x; read, write)` says which is which,
...
# dispatches: `dispatch!(p, fill_kernel!, (use(p, a; write = true), v), n)` is how
# a zeroed accumulator is declared, and nothing about it is special.
#
...
that is the thing no library announces: `use(p, C; write = true)` is why two
calls that write disjoint outputs from a shared weight do not serialise.
