...
# `vkformat` is NOT declared here, and not in `BACKEND_VOCABULARY`, for the
# reason CLAUDE.md gives for code paths: the vendor is in the name. A name only
# one backend answers, called only from that backend, is that backend's own
# function, like `mtlformat`. It is declared in `src/vulkan/lowering.jl`, beside
# the table it lowers.
