...
# There is no ambient "open batch" a launch can consult for where its commands
# go: a backend must not read task-global state to decide what to do with a
# dispatch, and the plan, the graph and the device are arguments at every call
# site. A caller that can say what its work touches declares a graph, which
# gives the same batching by construction — one command buffer per recording
