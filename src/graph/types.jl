...
a view is what the kernel GETS and the pass still touches the parent. So a view
handed to a dispatch is declared as a SLICE of its parent — scoped to the view's
elements, so two disjoint views of one buffer still get no barrier between them
— while the kernel receives the shaped window.
